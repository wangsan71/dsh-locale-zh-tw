		...
		*/
		function apply(ctx) {
			try {
				...
				// 0) publish 守衛：核心把主機設定視為事實來源——zh-TW 寫入主機會被
				//    schema（只認 zh/en）拒絕，客戶端恢復重載後 LocaleRuntime.adopt
				//    隨即 publish 回 zh/en，介面剛切繁就立刻被翻回簡（重新整理後的
				//    localStorage 還原也一樣會被蓋掉）。publish 是快照變更為數唯一的
				//    出口（setLocale / adopt / register 全走這裡），在此攔截：只要
				//    瀏覽器端偏好是 zh-TW，任何回退到別種語言的 publish 都改寫回 zh-TW。
				if (typeof locale.publish === "function" && locale.publish.__zhTwPatched !== true) {
					const originalPublish = locale.publish.bind(locale);
					const patchedPublish = function publish(active, localeChanged) {
						if (active !== "zh-TW" && hasZhTW() && readStoredLocale() === "zh-TW") active = "zh-TW";
						return originalPublish(active, localeChanged);
					};
					patchedPublish.__zhTwPatched = true;
					locale.publish = patchedPublish;
				}
				...
				// 3) setLocale：先把偏好寫進 localStorage 再 publish（守衛讀到新值才
				//    不會被回退）；zh-TW 完全不寫主機（寫了必被 schema 拒絕，並觸發
				//    恢復重載 → adopt 回退），只有 zh/en 照常持久化到主機設定。
				if (typeof locale.setLocale === "function" && locale.setLocale.__zhTwPatched !== true) {
					...
					const patchedSetLocale = function setLocale(id) {
						...
						if (match.id === "zh-TW") {
							if (locale.getLocale().active !== "zh-TW") locale.publish("zh-TW", true);
							return;
						}
						originalSetLocale(match.id);
					};
